...
class Result<T, E> with _$Result<T, E> {
  ...
  Result<U, E> flatMap<U>(Result<U, E> Function(T value) mapper) =>
      when(success: (value) => mapper(value), failure: Result.failure);
  ...
  T getOrElseGet(T Function(E error) onError) =>
      when(success: (value) => value, failure: (error) => onError(error));

  /// Returns the success value or null.
  T? getOrNull() => when(success: (value) => value, failure: (_) => null);

  /// Returns the error or null.
  E? errorOrNull() => when(success: (_) => null, failure: (error) => error);
}
